...
* **DB 마이그레이션 003:** 배포 직전 `alembic upgrade head`를 수행해 참가자 테이블과 relation aggregates 보강을 적용합니다. 문제 발생 시 `alembic downgrade 002`로 즉시 롤백하고 재배포 전에 self_other_schema_transaction.md §6의 복구 절차를 따릅니다.

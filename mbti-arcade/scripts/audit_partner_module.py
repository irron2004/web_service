"""Audit helper for the partner (romantic) perception module."""
...
def load_items(path: str | None, raw_json: str) -> list[dict]:
    if path:
        with open(path, "r", encoding="utf-8") as fh:
            return json.load(fh)
    if not raw_json:
        raise SystemExit("RAW_JSON is empty; provide --path or embed the JSON data")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit partner module question list")
    parser.add_argument("--path", help="Path to JSON file with partner items", default=None)
    args = parser.parse_args()

    items = load_items(args.path, RAW_JSON)
    audit(items)
